...
const calculateDiscount = expirationDate => {
  ...
  if (remainingDays <= 0) return 75; // Expired
  if (remainingDays <= 1) return 60; // 1 day left
  if (remainingDays <= 3) return 35; // 2-3 days left
  if (remainingDays <= 5) return 15; // 4-5 days left
